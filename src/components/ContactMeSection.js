...
const ContactMeSection = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { resetForm }) => {
      submit(values, {
        onSuccess: () => {
          onOpen('success', `Hi ${values.firstName}, your message has been sent!`);
          resetForm();
        },
        onError: (error) => {
  ...
  useEffect(() => {
    if (response) {
      if (response.type === 'success') {
        onOpen('success', `Hi ${formik.values.firstName}, your message has been sent!`);
        formik.resetForm();
  ...
  return (
    ...
    >
      <VStack w="1024px" p={32} alignItems="flex-start">
        <Heading as="h1" id="contactme-section" pt={{ base: "100px", md: "70px" }}>
          Contact me
        </Heading>
